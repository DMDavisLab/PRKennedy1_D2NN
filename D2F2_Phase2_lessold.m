...
% the ROI.
    ...
    if size(data_proc,1) > 1000000
        
        %add an index to data_proc
        data_proc(:,end+1) = 1:size(data_proc);
        
        number_of_chunks = ceil(sqrt(ceil(size(data_proc,1)/500000)))-1; % axial chunking, total sub-regions is the this squared (or cubed for 3D).
        chunk_width = (ceil((ProcSettings.PlotWidth / ceil(sqrt(ceil(size(data_proc,1)/10000)))-1)/100))*100;
        number_of_chunks = (ceil(ProcSettings.PlotWidth/chunk_width))^2;
        
        %meshgrid of xy starting coords for each region
        ...
        query_sub = zeros(number_of_chunks,4);
        for chunkID = 1:number_of_chunks
            query_sub(chunkID,1:2) = [X_query(chunkID),Y_query(chunkID)];
            query_sub(chunkID,3:4) = query_sub(chunkID,1:2) + chunk_width;
        end
        
        reference_sub = query_sub;
        reference_sub(:,1) = reference_sub(:,1) - chunk_width;
        reference_sub(:,2) = reference_sub(:,2) - chunk_width;
        reference_sub(:,3) = reference_sub(:,3) + chunk_width;
        reference_sub(:,4) = reference_sub(:,4) + chunk_width;
        
        % init the output tables
        ppFFD_inROI_tmp = NaN(size(data_proc,1),1);
        ppAFD_inROI_tmp = NaN(size(data_proc,1),1);
        DistancesToN_inROI = NaN(10,size(data_proc,1));

%         chunk_storage = struct;

        randbow = hsv(number_of_chunks^2);
        randbow = randbow(randperm(size(randbow,1)),:);
        progress_track = figure; %tracking figure
        axis([ProcSettings.AxisLimits(1)-chunk_width,ProcSettings.AxisLimits(2)+chunk_width,ProcSettings.AxisLimits(3)-chunk_width,ProcSettings.AxisLimits(4)+chunk_width]);

        for chunkID = 1:size(query_sub,1)
            curr_ref = rectangle('position',[reference_sub(chunkID,1),reference_sub(chunkID,2),chunk_width*3,chunk_width*3],'FaceColor',randbow(chunkID,:));
            curr_query = rectangle('position',[query_sub(chunkID,1),query_sub(chunkID,2),chunk_width,chunk_width],'FaceColor','w');
            drawnow
            
            Query_CropBounds = [query_sub(chunkID,1),query_sub(chunkID,3),query_sub(chunkID,2),query_sub(chunkID,4)];
            Chunk_query = RegionCropper(data_proc,Query_CropBounds,[1,2]);
                    
            if size(Chunk_query,1) > 0

                Ref_CropBounds = [reference_sub(chunkID,1),reference_sub(chunkID,3),reference_sub(chunkID,2),reference_sub(chunkID,4)];
                Chunk_ref = RegionCropper(QueryXYtemp,Ref_CropBounds,[1,2]);
            
               [ppFFD_chunk, ppAFD_chunk, DistsToN_chunk] = DTF2ParaFunc(Chunk_query,DTF2vars,Chunk_ref);

                % return results to approriate indices
                ppFFD_inROI_tmp(Chunk_query(:,3)) = ppFFD_chunk;
                ppAFD_inROI_tmp(Chunk_query(:,3)) = ppAFD_chunk;
                DistancesToN_inROI(:,Chunk_query(:,3)) = DistsToN_chunk;
            end
            
            set(curr_ref,'FaceColor',[0.5 0.5 0.5]);
            drawnow 
            
            clear ppFFD_chunk ppAFD_chunk DistsToN_chunk Chunk_query Chunk_ref
            
        end

        % clean up
        data_proc(:,end) = []; % remove the UID column
        delete(progress_track)
        clear query_sub reference_sub number_of_chunks chunkID randbow progress_track
...
%     if size(data_proc,1) > 1000000
%         
%         chunk_limits = 1:500000:size(data_proc);
%         number_of_chunks = size(chunk_limits,2);
%         chunk_storage = struct;
%         
%         % split data into manageable chunks
%         for chunkID = 1:number_of_chunks-1
%             chunk_storage.(['Chunk_',num2str(chunkID)]) = data_proc(chunk_limits(chunkID):chunk_limits(chunkID+1)-1,:);
%         end
%         chunkID = chunkID+1;
%         chunk_storage.(['Chunk_',num2str(chunkID)]) = data_proc(chunk_limits(chunkID):end,:);
%         
%         % init the output tables
%         ppFFD_inROI_tmp = [];
%         ppAFD_inROI_tmp = [];
...
%         for chunkID = 1:number_of_chunks
%             QueryXYtemp = RegionCropper(QueryXYtemp,[],[1,2]);
...
%             ppFFD_inROI_tmp = vertcat(ppFFD_inROI_tmp,chunk_storage.(['ppFFD_',num2str(chunkID)]));
%             ppAFD_inROI_tmp = vertact(ppAFD_inROI_tmp,chunk_storage.(['ppAFD_',num2str(chunkID)]));
%             DistancesToN_inROI = horzcat(DistancesToN_inROI,chunk_storage.(['DistsToN_',num2str(chunkID)]));
%         end
% 
...
% equal, i.e. EventOffset is zero.
    ...
    clear EventsOffset rndm_events_reqd ROIarea
